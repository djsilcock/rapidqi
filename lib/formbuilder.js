...
function BaseCheckboxComponent({ Component, validate, onClickFactory, numCols, options, name}) {
	...
	return useMemo(()=>{
		...
		return(<Grid stackable columns={numCols}>
			{cols}
		</Grid>)}
	,[options])
}
...
function makeComponentList(formdef) {
	...
	const components = formdef.map((fielddef) => {
		...
		fielddef.validate = (v) => fielddef.validation?.__isYupSchema__ ? fielddef.validation.validate(v).then(() => undefined, (e) => e.message) : fielddef.validation?.(v)
