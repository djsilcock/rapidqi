...
function TextAreaComponent({ name,validation }) {
	const validate = useCallback((v) => validation?.__isYupSchema__ ? validation.validate(v).then(() => undefined, (e) => e.message) : validation(v))
	return <FastField as={Form.TextArea} {...{name,validate}} />

}

const FormRow=React.memo(function ({ name, displayif, label, helptext, children, required, error, touched }) {

	displayif = displayif || (() => true)	
	if (!displayif(useFormikContext)) return null
	return (
		<Table.Row >
...
function makeComponentList(formdef) {
	...
	const components = formdef.map((fielddef) => {
		...
		const validate = useCallback((v) => fielddef.validation?.__isYupSchema__ ? fielddef.validation.validate(v).then(() => undefined, (e) => e.message) : fielddef.validation?.(v))
		return <FormRow key={fielddef.name} name={fielddef.name} label={fielddef.label} helptext={fielddef.helptext}>
			<Component key={fielddef.name} validate={validate} {...fielddef} />
		</FormRow>
	}
	)
